...
export function LeadCardContent({ lead }: LeadCardContentProps) {
  ...
  const nameField = leadFields.find((f: any) => name_keys.includes(f.name));
  const phoneField = leadFields.find((f: any) => phone_keys.includes(f.name));
  const emailField = leadFields.find((f: any) => f.name === "email");
