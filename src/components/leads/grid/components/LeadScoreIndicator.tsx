...
import { Flame, ThermometerSnowflake, ThermometerWarm } from "lucide-react";
...
export function LeadScoreIndicator({ leadId, className }: LeadScoreIndicatorProps) {
  ...
  switch (leadScore.category) {
    case 'hot':
      ...
      break;
    case 'warm':
      icon = <ThermometerWarm className="h-3 w-3 text-amber-500" />;
      colorClass = "bg-amber-100 text-amber-800 hover:bg-amber-200";
      label = "ליד בינוני";
      break;
    case 'cold':
      icon = <ThermometerSnowflake className="h-3 w-3 text-blue-500" />;
